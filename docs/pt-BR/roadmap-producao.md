...
| `m5-public-key-obrigatoria` | `update.public-key` obrigatória quando há bloco `update:`; inclui `config_template.dart:31-39` e todos os fixtures, senão todo projeto recém-gerado passa a ser recusado | M | — | ✔ só uma pessoa sabe qual chave o parque confia, e a privada dela não está nesta máquina |
