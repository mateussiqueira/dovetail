...
| O daemon no bundle, provado: `tool/ci/prove_service.sh` — um app descartável, `dovetail build` + `sign` com a identidade ad-hoc, e os seis fatos que o `SMAppService` precisa | ✅ feito | o plist, o `Label`, o `BundleProgram` relativo, o binário em `Contents/MacOS` e os entitlements próprios do daemon (sem `app-sandbox` herdado) tinham cada um teste de unidade com runner de gravação; nenhum provava que produziam um bundle junto. Agora é o último passo do `tool/release.sh`, então o release o carrega. O registro em si continua esperando um Developer ID |
...
| Orquestração de release — `tool/release.sh` rode os dois builds e os três canários na ordem | ✅ feito | cada passo já provou verde individualmente; o script é a ordem certa. O canário do daemon entrou quando provou o bundle e release nenhum o rodava |
| O componente privilegiado chega ao macOS: `service.macos` na configuração, o daemon embarcado no bundle, e os entitlements dele na assinatura | ✅ feito | o `dovetail_privileged_helper` só sabia responder "não instalado", porque ninguém punha o property list no `.app` e um `.dmg` não roda instalador. O bundler o embarca, o CLI o declara, e o `sign` lhe dá os entitlements que a chave prometia e nunca entregava |
| O Team ID do app e o do daemon embarcado são o mesmo | ✅ feito | o `sign` lê os dois com `codesign -dv` e recusa os dois divergindo, dizendo os dois valores e o caminho em que o daemon está — a recusa que a exigência produziria chega na máquina de quem usa, sobre a qual ele não pode agir. Provado contra o runner de gravação, como toda prova de assinatura aqui; o registro de um daemon de verdade ainda espera um Developer ID, que é o que a notarização precisa |
