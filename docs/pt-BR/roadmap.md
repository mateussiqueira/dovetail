...
| O Team ID do app e o do daemon embarcado são o mesmo | bloqueado | o `SMAppService` recusa um daemon cujo Team ID divirja do app, e recusa na máquina do usuário, com uma mensagem sobre a qual ele não pode agir. O `DarwinServiceRoute.bundled` escreve a exigência e nada a confere. Desbloqueia com um Developer ID real para assinar dois binários, que este host não tem — a checagem em si é `codesign -dv` nos dois e uma comparação |
