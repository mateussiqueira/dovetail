...
| O daemon no bundle, provado: `tool/ci/prove_service.sh` — um app descartável, `dovetail build` + `sign` com a identidade ad-hoc, e os seis fatos que o `SMAppService` precisa | ✅ feito | o plist, o `Label`, o `BundleProgram` relativo, o binário em `Contents/MacOS` e os entitlements próprios do daemon (sem `app-sandbox` herdado) tinham cada um teste de unidade com runner de gravação; nenhum provava que produziam um bundle junto. O registro em si continua esperando um Developer ID |
