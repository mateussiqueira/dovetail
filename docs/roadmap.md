...
| The Team ID of the app and of the embedded daemon are the same | blocked | `SMAppService` refuses a daemon whose Team ID differs from the app's, and it refuses at the user's machine with a message they cannot act on. `DarwinServiceRoute.bundled` writes the requirement down and nothing checks it. Unblocking needs a real Developer ID to sign two binaries with, which this host does not have — the check itself is `codesign -dv` on both and a comparison |
