...
| The daemon in the bundle, proved: `tool/ci/prove_service.sh` — a throwaway app, `dovetail build` + `sign` with the ad-hoc identity, and the six facts `SMAppService` needs | ✅ done | the plist, its `Label`, the relative `BundleProgram`, the binary in `Contents/MacOS` and the daemon's own entitlements (no inherited `app-sandbox`) each had a unit test with a recording runner; none proved they produce a bundle together. It is now the last step of `tool/release.sh`, so the release carries it. Registration itself still waits for a Developer ID |
...
| Release orchestration — `tool/release.sh` runs both builds and the three canaries in order | ✅ done | each step had already proven green on its own; the script is the right order. The daemon canary joined it when it had proven the bundle and no release ran it |
| The privileged component reaches macOS: `service.macos` in the config, the daemon embedded in the bundle, and its own entitlements at signing | ✅ done | `dovetail_privileged_helper` could only ever report "not registered", because nothing put the property list in the `.app` and a `.dmg` runs no installer. The bundler embeds it, the CLI declares it, and `sign` gives it the entitlements the key had always promised and never delivered |
| The Team ID of the app and of the embedded daemon are the same | ✅ done | `sign` reads both with `codesign -dv` and refuses the two diverging, naming both values and the path where the daemon sits — the refusal the requirement would otherwise produce arrives on the user's machine, about which they can do nothing. Proven against the recording runner, like every signing proof here; the registration of a real daemon still waits for a Developer ID, which is what notarisation needs |
